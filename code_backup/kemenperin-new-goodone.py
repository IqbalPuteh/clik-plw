...
async def scrape_table_data(page):
    ...
    try:
        ...
        for row in table_rows:
            ...
            if cells:
                if is_white_row:
                    # For white rows, create separate rows for each BR-separated segment
                    max_segments = 0
                    all_cell_segments = []
                    ...
                    for cell in cells:
                        ...
                        # Clean segments and keep structure
                        for segment in segments:
                            cleaned_segment = segment.strip()
                            cell_segments.append(cleaned_segment)  # Keep all segments, including empty ones for structure
                        
                        all_cell_segments.append(cell_segments)
                        max_segments = max(max_segments, len(cell_segments))
                    
                    # Create separate rows for each segment
                    for segment_index in range(max_segments):
                        ...
                        for cell_segments in all_cell_segments:
                            if segment_index < len(cell_segments):
                                ...
                                segment_row.append(segment_text)
                                if segment_text:  # Check if this segment has actual data
                                    has_data = True
                            else:
                                segment_row.append('')
        ...
        logger.info(f"Extracted {len(rows)} rows from current page (BR-separated)")
        return rows
...
def create_dataframe(data):
    ...
    for row in data:
        # Convert row to tuple for hashing (to detect duplicates)
    ...
    if header_row and data_start_index < len(unique_data):
        # Use the identified header row
        df = pd.DataFrame(unique_data[data_start_index:], columns=header_row)
    elif len(unique_data) > 1:
        # Fallback to using the first row as header
        df = pd.DataFrame(unique_data[1:], columns=unique_data[0])
    else:
    ...
    if not df.empty:
        ...
        # Remove duplicate rows in DataFrame as well
